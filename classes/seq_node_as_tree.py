...
class SequenceNodeAsTree:
    ...
    def find_event_sub_type(self, event: IndelEvent) -> tuple[EventSubTypes, AVLNode, int]:
        node_at_inx, seq_length_with_block = self.find_block_item_and_sites_count(event.place)
        seq_len_up_to_block: int = (seq_length_with_block - node_at_inx.bl.inserted_seq_count -
                                    node_at_inx.bl.copy_sites_count)
        if event.length < 0 or event.place > seq_length_with_block:
            return EventSubTypes.OUT_OF_SEQUENCE, node_at_inx, seq_length_with_block
        if event.is_insertion:
            if node_at_inx is None:
                return EventSubTypes.INSERTION_AT_END, node_at_inx, seq_length_with_block
            if event.place == 0:
                return EventSubTypes.INSERTION_AT_START, node_at_inx, seq_length_with_block
            if event.place < seq_len_up_to_block + node_at_inx.bl.copy_sites_count:
                return EventSubTypes.INSERTION_INSIDE_COPIED, node_at_inx, seq_length_with_block
            return EventSubTypes.INSERTION_INSIDE_INSERTED, node_at_inx, seq_length_with_block
        else:
            if node_at_inx.bl is None and seq_length_with_block == event.place:
                return EventSubTypes.OUT_OF_SEQUENCE, node_at_inx, seq_length_with_block
            ...
            if position_in_block <= node_at_inx.bl.copy_sites_count:
                if position_in_block + event.length <= node_at_inx.bl.copy_sites_count:  # contained in copy_sites_count
                    if position_in_block > 0:
                        return EventSubTypes.DELETION_INSIDE_COPIED_CONTAINED, node_at_inx, seq_length_with_block
                    return EventSubTypes.DELETION_OF_COPIED, node_at_inx, seq_length_with_block
                return EventSubTypes.DELETION_INSIDE_COPIED_UNCONTAINED, node_at_inx, seq_length_with_block
            if position_in_block + event.length <= node_at_inx.bl.inserted_seq_count:  # contained in inserted
                if position_in_block > node_at_inx.bl.copy_sites_count:
                    return EventSubTypes.DELETION_INSIDE_INSERTED_CONTAINED, node_at_inx, seq_length_with_block
                return EventSubTypes.DELETION_INSIDE_INSERTED_UNCONTAINED, node_at_inx, seq_length_with_block
            return EventSubTypes.DELETION_OF_INSERTED, node_at_inx, seq_length_with_block
