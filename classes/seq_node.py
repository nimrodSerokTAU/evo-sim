...
class SequenceNode:
    blck_list: list[BlockListItem]
    my_length: int

    def __init__(self, original_sequence_length: int):
        self.my_length = original_sequence_length
        self.blck_list = [BlockListItem(index_in_predecessor=0, copy_sites_count=self.my_length,
    ...
    def find_block_index_and_sites_count(self, place: int) -> tuple[int, int]:
        ...
        for i in range(len(self.blck_list)):
            this_block: BlockListItem = self.blck_list[i]
    ...
    def find_event_sub_type(self, event: IndelEvent) -> tuple[EventSubTypes, int, int]:
        ...
        block_at_inx: BlockListItem = self.blck_list[cb_index]
    ...
    def calculate_insertion_event(self, event: IndelEvent, event_type: EventSubTypes, block_at_inx: BlockListItem,
        ...
        if event_type == EventSubTypes.INSERTION_AT_END:
            ...
            block_item = BlockListItem(index_in_predecessor=0,
                                       copy_sites_count=0,
            ...
            block_item = BlockListItem(index_in_predecessor=block_at_inx.index_in_predecessor + first_block_copy_count,
    ...
    def calculate_deletion_event(self, event: IndelEvent, event_type: EventSubTypes, block_at_inx: BlockListItem,
                                 cb_index: int, seq_length_with_block: int):
        seq_len_up_to_block: int = (seq_length_with_block - block_at_inx.inserted_seq_count -
                                    block_at_inx.copy_sites_count)
        position_in_block: int = event.place - seq_len_up_to_block
        if event_type == EventSubTypes.DELETION_INSIDE_COPIED_CONTAINED:
            block_item = BlockListItem(
    ...
    def delete_from_insertion_part(self, block: BlockListItem, deletion_len: int, deleted_from_insertion: int,
